...
def getNewIndex(nodeindex, adjacency_matrix):
    A = adjacency_matrix
    nodeset = []
...
def evalParams(params, config, device, logger, targetDataset, writer, epoch, basemodel):
    ...
    for dataset in datasetsall:
        ...
        step = 0
        logger.info('parameter evaluation on {}: {} nodes'.format(dataset, node_num))
        params = params.reshape((params.shape[0],-1))
        model = StgnnSet(data_args, task_args, model_args, model=basemodel).to(device=device)
        adjacency_matrix = np.load(data_args[dataset]['adjacency_matrix_path'])
        for node_index in tqdm(range(node_num), desc='eval {}'.format(dataset)):
            ...
            newindex = getNewIndex(node_index, adjacency_matrix)
