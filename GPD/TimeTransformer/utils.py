...
class XEDataset(Dataset):
    ...
    def __init__(self, mparams, condition):
        self.mparams = torch.as_tensor(mparams, dtype=torch.float32)
        self.condition = torch.as_tensor(condition, dtype=torch.float32)

    def __getitem__(self, index):
        return self.mparams[index], self.condition[index]
