...
def load_prediction_config():
    config_path = Path(__file__).resolve().parent.parent / 'PredictionModel' / 'config.yaml'
...
class GaussianDiffusion1D(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, condition, x_self_cond=None, clip_denoised=True):
     
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device=x.device, dtype=torch.long)  
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x=x, t=batched_times, condition=condition, x_self_cond=x_self_cond,
    ...
    def p_sample_loop(self, shape, condition):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, condition, self_cond, clip_denoised=True) 
    ...
    def forward(self, mparam, condition, *args, **kwargs):
        ...
        return self.p_losses(mparam, t, condition, *args, **kwargs)  
...
class Trainer1D(object):
    def __init__(
        ...
        **_,
        ...
        dl = DataLoader(dataset, batch_size = train_batch_size, shuffle = True, pin_memory = True, num_workers = 0, drop_last=True)
    ...
    def sample_condition_batches(self, sample_model, condition, device, stage_name='sampling'):
        total = len(condition)
        if self.sample_batch_size <= 0:
            chunk_size = total
        else:
            chunk_size = min(self.sample_batch_size, total)

        if chunk_size < total:
            chunk_count = (total + chunk_size - 1) // chunk_size
            self.logger.info(
                '{}: generating {} samples in {} chunks (chunk size {})'.format(
                    stage_name, total, chunk_count, chunk_size
                )
            )
        else:
            self.logger.info('{}: generating {} samples in one chunk'.format(stage_name, total))

        result = None
        for start in range(0, total, chunk_size):
            end = min(start + chunk_size, total)
            sampled_seq = sample_model.sample(
            ...
            if result is None:
                ...
                result = np.concatenate((result, sampled_seq), axis=0)
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        loss = self.model(mparam, condition) 
                    ...
                    if self.step % self.save_and_sample_every == 0:  
                        ...
                        with torch.no_grad():
                            ...
                            for _ in range(self.sampleTimes):
                                result = self.sample_condition_batches(
                                    self.ema.ema_model,
                                    self.condition,
                                    device,
                                    stage_name='eval sampling'
                                )
